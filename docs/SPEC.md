...
| 2 | Migrations, tables, indexes, RLS | Schema live. Migrations 0006–0021 are in the repo. **Cross-org RLS test PASS** on production 2026-09-26: 31 tables, 161 checks, 0 failures; negative control DETECTS_LEAK; nothing left behind (`supabase/tests/`, `npm run test:rls`) | 0001–0005 are not in the repo (applied earlier) |
...
| 7 | History, Audit, JSON export, Settings | **History**: the DECISION → APPROVAL → RUN → ACTUAL → VERIFICATION chain per run plus one event list (who, when), built only from stored rows; a missing step is NOT AVAILABLE (`lib/history/timeline.ts`, 3 tests). **Audit** (migration 0013): `seal_run_audit` (ADMIN/ENGINEER) has the database build a snapshot of the run's evidence, hash it (SHA-256) and chain it to the previous record (seq + previous_hash); `audit_integrity` re-checks hash and chain on every view; print view; TRUNCATE revoked from client roles. **JSON export** `/api/audit/[id]/export` under RLS, allowlist only, unknown fields dropped and counted (`lib/audit/export.ts`, 5 tests, incl. allowlist = keys built by 0013). **Settings**: organization, sites, members with names, permission matrix, configuration (engine status, ruleset v0-uncalibrated, gates version). DB tests PASS on production (rolled back): audit seal 16/16 (incl. edited, re-hashed and removed record detected), permission matrix 12 actions × 4 roles, 0 mismatches | Inviting users and changing roles in the app (needs Auth admin); logged-in rendering not verified end-to-end; nothing to audit until real runs exist |
| 8 | Full audit AR 1–20, AT report | Done: `docs/AUDIT_AR_AT.md`. Now 15 PASS, 5 PARTIAL, 0 FAIL, after fixes 1–4:<ul><li>0014: APPROVAL → RUN;</li><li>0015/0016: engine write path;</li><li>0017: contract audit export;</li><li>term scan in the build.</li></ul> | NOT RUN, waiting on secrets:<ul><li>live engine (URL, token, write key);</li><li>real protected terms (`TERM_GUARD_KEY` + digests).</li></ul> |
...
| FAIL loop, working canon (`docs/CANON.md`): ROZPAD I → diagnosis → 3 → 6 → 28 → refresh → repair → test → verification | Done (0019–0021, `docs/FAIL_LOOP.md`): append-only steps with checked references; 6 is a recorded exclusion, never a delete |
...
| 38 FILTR DOWODÓW (gate) → 39 WERYFIKACJA + UTRWALENIE → 40 LOCK / READ_ONLY → CROSS → AUDIT | Done (0021): 38 passes a verified PASS with evidence or closes the case (never knowledge); 39 re-checks and persists; 40 = verified, persisted, locked (no numeric threshold); seal audit-v3 contains the case. Canon B (CZARNA WODA, KHRR) not defined, not built |
