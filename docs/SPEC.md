...
| 2 | Migrations, tables, indexes, RLS | Schema live. Migrations 0006–0014 are in the repo. **Cross-org RLS test PASS** on production 2026-09-25: 28 tables, 146 checks, 0 failures; negative control DETECTS_LEAK; nothing left behind (`supabase/tests/`, `npm run test:rls`) | 0001–0005 are not in the repo (applied earlier) |
...
| 8 | Full audit AR 1–20, AT report | Done: `docs/AUDIT_AR_AT.md`. Now 14 PASS, 4 PARTIAL, 2 FAIL, after fix 1 (0014: APPROVAL → RUN enforced by the database, run lifecycle UI, test 20/20) | FAIL: AR-7 (the app never requests an engine decision), AR-18 (no protected-term scan of build output, messages or API). Fix list in the report |
