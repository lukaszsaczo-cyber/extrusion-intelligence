...
| 2 | Migrations, tables, indexes, RLS | Schema live. Migrations 0006–0013 are in the repo. **Cross-org RLS test PASS** on production 2026-09-25: 28 tables, 146 checks, 0 failures; negative control DETECTS_LEAK; nothing left behind (`supabase/tests/`, `npm run test:rls`) | 0001–0005 are not in the repo (applied earlier) |
...
| 8 | Full audit AR 1–20, AT report | Done: `docs/AUDIT_AR_AT.md`. 12 PASS, 5 PARTIAL, 3 FAIL. All tests re-run 2026-09-25 (database tests on production, rolled back) | FAIL: AR-7 (the app never requests an engine decision), AR-9 (a run is accepted on an unapproved plan), AR-18 (no protected-term scan of build output, messages or API). Fix list in the report |
