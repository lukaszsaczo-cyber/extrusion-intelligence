-- 0021: working canon (2026-09-26). The separate RAPORT step is removed: after
-- WERYFIKACJA comes 38 FILTR DOWODÓW, the evidence gate before 39 and 40. It either
-- passes items to 39 or records the result and closes the case (never knowledge).
-- Step codes: 3 ODDZIELENIE, 6 FILTR / WYKLUCZENIE (status change, raw data kept),
-- 28 KONSOLIDACJA, 38 FILTR DOWODÓW, 39 WERYFIKACJA + UTRWALENIE, 40 LOCK / READ_ONLY
-- (verified, persisted and locked; no numeric threshold). Canon B is not built.

do $$ begin
  if exists (select 1 from public.fail_case_steps) then
    raise exception 'FAIL loop steps are not empty; removing REPORT needs a data migration';
  end if;
end $$;

drop function public.record_fail_step(uuid, public.fail_step, uuid, jsonb);
drop function private.fail_latest(uuid, public.fail_step);
drop function private.fail_last_step(uuid);

alter type public.fail_step rename to fail_step_0019;
create type public.fail_step as enum (
  'DECOMPOSITION',     -- ROZPAD I
  'DIAGNOSIS',         -- SZCZEGÓŁOWA DIAGNOZA
  'EXTRACT',           -- 3  ODDZIELENIE: correct vs erroneous, nothing repaired yet
  'PURGE',             -- 6  FILTR / WYKLUCZENIE: mark what 3 found erroneous as excluded (never delete)
  'CONSOLIDATE',       -- 28 KONSOLIDACJA: the kept parts as one stable base state
  'STATE_REFRESH',     -- ODŚWIEŻENIE STANU
  'INTERVENTION',      -- NAPRAWA
  'CONTROLLED_TEST',   -- TEST
  'VERIFICATION',      -- WERYFIKACJA
  'FILTER',            -- 38 FILTR DOWODÓW: the gate before 39/40
  'VERIFY_PERSIST',    -- 39 WERYFIKACJA + UTRWALENIE
  'LOCK',              -- 40 LOCK / READ_ONLY
  'CROSS',             -- CROSS: a new cycle starts from the locked state
  'AUDIT');            -- AUDIT
alter table public.fail_case_steps alter column step type public.fail_step using step::text::public.fail_step;
drop type public.fail_step_0019;

create function private.fail_last_step(p_case uuid)
...
$$;
create function private.fail_latest(p_case uuid, p_step public.fail_step)
...
-- knowledge only from a verified PASS with saved evidence that passed the 38 gate
...
begin
  ...
  if c.outcome is distinct from 'VERIFIED_PASS' or not coalesce(c.outcome_evidence_saved, false)
     or not exists (select 1 from public.fail_case_steps s where s.case_id = new.case_id and s.step = 'FILTER'
                    and s.payload ->> 'result' = 'PASSED') then
...
begin
  ...
  -- again on the refreshed clean state (never a forced cause). After WERYFIKACJA always
  -- 38, the evidence gate: it passes to 39 or closes the case (never knowledge).
  if not coalesce((v_last.step = 'DECOMPOSITION' and p_step = 'DIAGNOSIS')
       ...
       or (v_last.step = 'VERIFICATION' and p_step = 'FILTER')
  ...
  if p_step = 'DIAGNOSIS' then
    ...
    -- 38 FILTR DOWODÓW: the gate before 39/40. Passes only items that were erroneous at 3
    -- and are now a verified PASS with saved evidence, with the change that did it.
    -- Anything else (FAIL, INCONCLUSIVE, INCOMPLETE, no evidence, nothing repaired)
    -- is recorded here and closes the case: a FAIL is never knowledge.
    select * into v_x from private.fail_latest(p_case, 'EXTRACT');
    v_rows := case when c.outcome = 'VERIFIED_PASS' and c.outcome_evidence_saved then
                coalesce((select jsonb_agg(x order by x ->> 'parameter')
    ...
    v_payload := jsonb_build_object(
      'result', case when jsonb_array_length(v_rows) > 0 then 'PASSED' else 'NOT_PASSED' end,
      'passed', v_rows,
      'outcome', c.outcome, 'evidence_saved', c.outcome_evidence_saved,
    ...
    v_close := jsonb_array_length(v_rows) = 0;
...
-- An attempt can be closed before WERYFIKACJA with a reason (kept, never knowledge).
...
begin
  ...
  if c.status <> 'OPEN' then raise exception 'fail_closed: the case is closed' using errcode = '23514'; end if;
  if c.outcome is not null then raise exception 'fail_order: after WERYFIKACJA the case goes through 38' using errcode = '23514'; end if;
