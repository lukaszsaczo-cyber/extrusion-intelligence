...
-- ENGINEER seal, the snapshot (audit-v3, 0019) is built by the database, the chain links every
...
declare
  ...
  expected_keys text[] := array['diagnoses','fail_cases','files','machine','measurements','organization','plan','predictions','previous_hash','quality',
...
begin
  ...
  if (select array_agg(k order by k) from jsonb_object_keys(snap) k) <> expected_keys then
    failures := failures || jsonb_build_object('check', 'snapshot has exactly the audit-v3 keys', 'keys', (select jsonb_agg(k) from jsonb_object_keys(snap) k));
  end if;
  checks := checks + 1;
  if not (snap ->> 'seq' = '1' and snap -> 'previous_hash' = 'null'::jsonb and snap ->> 'sealed_by' = u_eng::text
          ...
          and snap ->> 'schema' = 'audit-v3' and snap -> 'fail_cases' = '[]'::jsonb and snap #>> '{organization,name}' = 'Audit test' and snap #>> '{site,name}' = 's'
